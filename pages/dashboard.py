...
def _(descriptions_tables, mo, tab_fetch, tab_update):
    ...
    user_tabs = mo.ui.tabs(
        {"Voir les informations": tab_fetch, "Mettre à jour": tab_update, "Descriptions": descriptions_tables},
        value=get_tab(),
        on_change=set_tab,
    )
...
@app.cell
def _(client_id, fc, tab_update):
    if tab_update.value is not None:
        fc.update_client_info(client_id, tab_update.value)
    return
...
def _(client_id, go, mo, resp):
    ...
    return (jauge,)  # pyright: ignore[reportPossiblyUnboundVariable]


@app.cell
def _(client_id, jauge, mo):
    _output = None
    if client_id:
...
def _(client_id, fc, mo, pd, px, resp):
    ...
    return (local_plot,)  # pyright: ignore[reportPossiblyUnboundVariable]
...
def _(client_id, fc, mo, resp):
    ...
    return (waterfall,)  # pyright: ignore[reportPossiblyUnboundVariable]
